...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if fn == "gen_enroll_cert" { // 임치증명서 발급
		return s.gen_enroll_cert(stub, args)
	} else if fn == "gen_cont_cert" { // 계약증명서 발급
		return s.gen_cont_cert(stub, args)
...
//	client := map[string]int{args[6] : cont_term} // client라는 연관 데이터 타입(map) 생성(client[회사명] = 회사별임치기간)
...
//	protect.Client = append(protect.Client, map[string]int{args[1] : cont_term})
...
// 임치증명서 발급
func(s *SmartContract) gen_enroll_cert(stub shim.ChaincodeStubInterface, args []string) sc.Response{
	...
	technology := args[0]
	fmt.Printf("- start gen_enroll_cert : %s\n", technology)
	...
	fmt.Printf("- generate enrollment certification returning: \n%s\n", buffer.String())

	return shim.Success(buffer.Bytes())
}

// 계약증명서 발급
func(s *SmartContract) gen_cont_cert(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	stub.GetTxID()
	return shim.Success(nil)
}
