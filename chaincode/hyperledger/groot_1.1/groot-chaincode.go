...
type Protect struct {   
	...
	Content			[]string	   `json:"content"`		// 임치 데이터의 file별 내용에 대한 hash값(map)
...
func(s *SmartContract) add_cont(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if len(args) != 8 { // web에서 값 넘겨받기
		return shim.Error("Incorrect number of parameters. Expecting 8")
	}
	...
	if len(args[6]) <= 0 {
		return shim.Error("7th argument Enroll_date must be a non-empty string\n")
	}
	if len(args[7]) <= 0 {
		return shim.Error("8th argument Status must be a non-empty string\n")
	}
	...
	enroll_date := args[6]
	status, err := strconv.Atoi(args[7]) // status값 순자로 변환(상태)
	if err != nil {
		return shim.Error("8th argument Status must be a numeric string\n")
	}
