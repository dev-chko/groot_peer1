...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if fn == "generate_cert" { // 임치 및 계약증명서 발급
...
func(s *SmartContract) add_cont(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if len(args) != 10 { // web에서 값 넘겨받기
		return shim.Error("Incorrect number of parameters. Expecting 10")
	}
	...
	if len(args[8]) <= 0 {
		return shim.Error("9th argument Enroll_date must be a non-empty string\n")
	}
	if len(args[9]) <= 0 {
		return shim.Error("10th argument Status must be a non-empty string\n")
	}
	...
	if err != nil {
		return shim.Error("8th argument Contract term must be a numeric string\n")
	}
	client := make(map[string]int)
	client[args[6]] = cont_term // client라는 연관 데이터 타입(map) 생성(client[회사명] = 회사별임치기간)
	enroll_date := args[8]
	status, err := strconv.Atoi(args[9]) // status값 순자로 변환(상태)
	if err != nil {
		return shim.Error("10th argument Status must be a numeric string\n")
	}
...
func(s *SmartContract) add_content(stub shim.ChaincodeStubInterface, args []string) sc.Response {
        // check parameter(Technology, Content, Status) : 기술명을 입력받아 해당 기술의 내용을 추가
        if len(args) != 3 {
                return shim.Error("Incorrect number of arguments. Expecting 3")
        }
        ...
        if len(args[1]) <= 0 {
                return shim.Error("2nd argument Content must be a non-empty string\n")
        }
        if len(args[2]) <= 0 {
                return shim.Error("3nd argument Status must be a non-empty string\n")
        }
...
func(s *SmartContract) generate_cert(stub shim.ChaincodeStubInterface, args []string) sc.Response{
	...
	technology := args[0]
	fmt.Printf("- start generate_cert : %s\n", technology)
//	status, err := strconv.Atoi(args[1]) // status 값 숫자로 변환(상태값)
//        if err != nil {
//                return shim.Error("2th argument Status must be a numeric string\n")
//        }
	...
	fmt.Printf("- generate certification returning: \n%s\n", buffer.String())
...
func(s *SmartContract) get_tech(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	doc, _ := json.MarshalIndent(protectBytes, "", "  ")
...
//	buffer.WriteString("\"{")
//
//	for _, value := range protect.Content {
//		if checkcomma == true {
//			buffer.WriteString(",")
//		}
//		buffer.WriteString(value)
...
//	buffer.WriteString("\"")
//	buffer.WriteString(protect.Status)
//	buffer.WriteString("\"")
//	buffer.WriteString("}")
//
	/*	
